...
class CalendarManagement extends Component {
    ...
    ajaxGet("perms")
      .then((res) => {
        ...
        for (let index = 0; index < perms.length; index++) {
          for (
            let index_creneau = 0;
            index_creneau < perms[index].creneaux.length;
            index_creneau++
          ) {
            // Créneau au format AAAA-MM-dd:Creneau:id
            const creneau_array =
              perms[index].creneaux[index_creneau].split(":");
            let creneau = {
              id: creneau_array[2],
  ...
  createCreneaux(date, existing_creneaux) {
    ...
    const creneau_matin_index = existing_creneaux.findIndex(
      (c) => this.compareDates(c.date, date) && c.creneau === "M",
    );
    if (creneau_matin_index >= 0) {
      creneaux.matin = existing_creneaux[creneau_matin_index];
    } else {
      creneaux.matin = {
        date: this.formatCreneauDate(date),
        creneau: "M",
        creneau_detail: "Matin",
        perm_id: "",
      };
    }

    const creneau_midi_index = existing_creneaux.findIndex(
      (c) => this.compareDates(c.date, date) && c.creneau === "D",
    );
    ...
    const creneau_soir_index = existing_creneaux.findIndex(
      (c) => this.compareDates(c.date, date) && c.creneau === "S",
    );
  ...
  handlePlanningChange(event, week_index, day_index, creneau_type) {
    ...
    calendar[week_index][day_index].creneaux[creneau_type].perm_nom =
      perms[perm_index].nom;
    calendar[week_index][day_index].creneaux[creneau_type].perm_id = perm_id;
    ajaxPost("creneau/", calendar[week_index][day_index].creneaux[creneau_type])
      .then((res) => {
        const creneau_date =
          calendar[week_index][day_index].creneaux[creneau_type].date;
        const creneau_period =
          calendar[week_index][day_index].creneaux[creneau_type].creneau;
        calendar[week_index][day_index].creneaux[creneau_type].id = res.data.id;
        perms[perm_index].creneaux.push(
          creneau_date + ":" + creneau_period + ":" + res.data.id,
        );
        this.setState({
          calendar: calendar,
          perms: perms,
  ...
  handleDeleteCreneau(event, week_index, day_index, creneau_type, perm_id) {
    let calendar = [...this.state.calendar];
    let perms = [...this.state.perms];
    let perm_index = perms.findIndex(
      (p) => p.id.toString() === perm_id.toString(),
    );

    const creneau = calendar[week_index][day_index].creneaux[creneau_type];

    ajaxDelete("creneau/" + creneau.id)
      .then((res) => {
        ...
        perms[perm_index].creneaux = perms[perm_index].creneaux.filter(
          (c) => c !== creneau.date + ":" + creneau.creneau + ":" + creneau.id,
        );
        this.setState({
          calendar: calendar,
          perms: perms,
  ...
  autoCompleteQuery(query, autocomplete_type) {
    ajaxGet("payutc/user/autocomplete/" + query)
      .then((res) => {
  ...
  handleChangeOnMail() {
    ...
    ajaxGet("perms")
      .then((res) => {
        this.setState({ unselected_perms: res.data });
      })
      .catch((error) => {
        console.log(error);
      });
  ...
  selectPerm(event, perm) {
    ...
    this.setState({
      ...
      unselected_perms: unselected_perms,
    });
  }
  ...
  sendPermsMail() {
    ...
    ajaxPost("perms/mail", this.state.selected_perms)
      .then((res) => {
      ...
      <div className="admin_container">
          ...
          <Paper className={classes.paper_box}>
            <List className={classes.listPerms}>
              {perms.map((perm, index) => {
                const M_creneaux = perm.creneaux.filter(
                  (c) => c.split(":")[1] === "M",
                ).length;
                const D_creneaux = perm.creneaux.filter(
                  (c) => c.split(":")[1] === "D",
                ).length;
                const S_creneaux = perm.creneaux.filter(
                  (c) => c.split(":")[1] === "S",
                ).length;
                  ...
                  <React.Fragment key={index}>
                    ...
                    >
                      <ListItemText
                        ...
                        secondary={
                          perm.nom_resp + (perm.asso ? " - Association" : "")
                        }
                      />
                      {canDelete && (
            ...
            <Table size="small">
              <TableHead>
                <TableRow>
                  ...
                  <TableCell className="center side_padding8">
                    Mercredi
                  </TableCell>
                        ...
                        <div className={classes.creneau_card}>
                          ...
                          {day.creneaux.matin.perm_id ? (
                            <Chip
                              ...
                              onDelete={
                                this.isDatePast(day.date)
                                  ? null
                                  : (e) =>
                                      this.handleDeleteCreneau(
                                        e,
                                        index,
                                        index_day,
                                        "matin",
        ...
        >
          ...
          <DialogContent>
            <Paper className={classes.paper}>
              <DialogContentText>
                Ci-dessous la liste des perms auxquelles doit être envoyées un
                mail de notification des plannings.
              </DialogContentText>
            ...
            <Paper className={classes.paper}>
              <DialogContentText>
                Ci-dessous la liste des perms auxquelles{" "}
                <strong>ne doit pas</strong> être envoyées un mail de
        ...
        >
          ...
          <DialogActions>
            <Button
              onClick={() => {
                this.setState({ mail_sent_dialog: false });
              }}
              color="secondary"
              variant="contained"
              size="small"
